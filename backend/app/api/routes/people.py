...
from app.db.models import Person, Image, User
        ...
        if p.faces:
            ...
            if best_face and best_face.image:
                ...
                if not cover:
                    ...
                    cover = f"/api/v1/images/content/{best_face.image.id}"
...
@router.post("/merge")
def merge_people(request: MergeRequest, session: Session = Depends(get_session), current_user: User = Depends(security.get_current_user)):
